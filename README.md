...
- **隐私优先**：139 Token 使用 AES-256-GCM 认证加密；无注册登录，只有主动使用 AI 拍题时才访问 139 官方接口
...
默认端口是 `18666`。启动时会自动检测 Tailscale IPv4，并只监听该地址；请使用启动日志中显示的地址访问，例如：

```text
http://100.x.x.x:18666
```

同一 Tailnet 中的电脑、手机和平板均可使用该地址访问。这样不会默认把含本地错题和 AI Token 的页面暴露给其他网卡。

没有安装或未连接 Tailscale 时，服务会回退到 `127.0.0.1:18666`。
...
```bash
MISTAKE_BOOK_HOST=0.0.0.0 MISTAKE_BOOK_PORT=18666 bash "run.sh"
```
...
└── .credential.key   # 本地主密钥，权限 0600
```

“AI 与数据 → 下载完整备份 ZIP”会包含上述目录。恢复时停止服务，将备份中的 `data/` 放回项目目录即可。覆盖现有数据前请自行备份。
